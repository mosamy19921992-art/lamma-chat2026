-- Expose store-facing owner fields on public_chat_settings (safe for all members).
-- payment_info + custom_features are needed for VIP checkout — not admin secrets.
...
as $$
  select jsonb_build_object(
    ...
    'custom_features', coalesce(p_row.custom_features, '[]'::jsonb),
    'payment_info', coalesce(p_row.payment_info, '{}'::jsonb),
